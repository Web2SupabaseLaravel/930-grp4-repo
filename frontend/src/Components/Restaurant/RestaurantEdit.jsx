        ...
        <div className="container mt-4 d-flex">
            <div className='col-sm-8'>
                <div className='d-flex justify-content-between mb-4'>
                    ...
                    <i className="bi bi-pencil-square fs-3 me-3 "></i>
                </div>
                <hr />
            ...
            <div className='col-sm-4 ms-5 mt-1'>
                <div className='d-flex justify-content-between mb-4'>
                    ...
                    <i className="bi bi-align-top fs-3 me-3"></i>
                </div>
                <hr />
                ...
                <div className="mt-4">
                    <h5 className='fw-bold sansita text-secondary'>Current Tables</h5>
                    <hr />
                    <div className="row  ">
                        {tables.map((table, idt) => (
                            <div key={idt} className={` card mb-3`} style={{ minHeight: '80px', maxHeight: '80px', minWidth: '29rem', maxWidth: '29rem' }}>
                                <div className={"card-body resbutton d-flex  justify-content-between align-items-center "}>
                                    <h5 className="me-3 card-title resbutton text-center fs-5   "> <strong> {idt + 1}</strong></h5>
                                    <h6 className=" resbutton me-1 text-center ">{table.location}</h6>
                                    <p className='card-text mb-1 ms-3 me-3'><strong>Size:</strong> {table.size} people</p>
                                    <p className={`badge ${table.status === '1' ? 'bg-success' : 'bg-secondary'} pt-2 pb-2 m-2`}>{(table.status === '1') ? "Available" : "Occupied"}</p>
                                    <button className="btn btn-dark resbutton pt-1 pb-1" onClick={() => {
                                ...
                                {editingTableId === table.id && (
                                    <form
                                        className="mt-3 zform"
                                        onSubmit={(e) => {
                                            e.preventDefault();
                                            axios.put(`http://127.0.0.1:8000/api/restaurants/${id}/tables/${editingTableId}`, {
                                                size: parseInt(editTableForm.size),
                                                status: editTableForm.status ? 1 : 0,
                                                location: editTableForm.location
                                            })
                                                .then(res => {
                                                    setTables(prev =>
                                                        prev.map(t => t.id === editingTableId ? res.data : t)
                                                    );
                                                    setEditingTableId(null);
                                                    showMessage('Table updated successfully!');
                                                })
                                                .catch(err => {
                                                    showMessage('Failed to update table', 'danger');
                                                });
                                        }}
                                    >
                                        <div className="mb-2">
                                            ...
                                            <input
                                                type="number"
                                                className="form-control"
                                                value={editTableForm.size}
                                                onChange={(e) => setEditTableForm({ ...editTableForm, size: e.target.value })}
                                            />
                                        </div>
                                        <div className="mb-2">
                                            ...
                                            <input
                                                ...
                                                onChange={(e) => setEditTableForm({ ...editTableForm, location: e.target.value })}
                                            />
                                        </div>
                                        <div className="form-check mb-2">
                                            <input
                                                type="checkbox"
                                                className="form-check-input"
                                                checked={editTableForm.status}
                                                onChange={(e) => setEditTableForm({ ...editTableForm, status: e.target.checked })}
                                            />
                                            <label className="form-check-label">Available</label>
                                        </div>
                                        <div className="d-flex gap-2">
                                            <button className="btn btn-dark resbutton btn-sm" type="submit">Save</button>
                                            <button
                                                className="btn btn-secondary resbutton btn-sm"
